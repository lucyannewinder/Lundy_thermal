...
## sample_images
Contains a sample of thermal images used in this study. There are 30 images which correspond to 8 nestlings from 2 broods at all ages each individual was alive for. These 2 broods had nestlings cross-fostered between them and has individuals at each age so is representative of the larger dataset. The folder also contains the associated data for these individuals (Tb.dat_IRsubset.csv), which is a subset of Tb.dat.csv in Data/Raw. The data contains an additional column (Photo.id) which associates each image to each observation for a given individual.
